...
class RemovePERepoPlatformClasses < TaskHelper
  def task(_params)
    ...
    removed_classes = platform_classes.clone
    remove_classes!(pe_master_group, platform_classes)
  ...
  def remove_classes!(pe_group, classes)
    classes.each { |key| pe_group['classes'].delete(key) }

    update_payload = {
      'id' => pe_group['id'],
      'name' => pe_group['name'],
      'parent' => pe_group['parent'],
      'classes' => pe_group['classes'],
    }

    response = update_group(pe_group['id'], update_payload)
    ...
    { status: 'success', message: 'Removed platform classes successfully.' }
  end

  def update_group(group_id, group_data)
    net = https(4433)
    request = Net::HTTP::Put.new("/classifier-api/v1/groups/#{group_id}", { 'Content-Type' => 'application/json' })
